...
| N //代表 None/null
    ...
    static member Compare (x: fCell2<'CellTupleKey>, y: fCell2<'CellTupleKey>): int =
        ...
        | null, null -> 0
        | _, null ->
            if x = S null || x = fCell2.ANull || x = N then 0 else 1
        | null, _ ->
            if y = S null || y = fCell2.ANull || y = N then 0 else -1
        | _ ->
            match (x, y) with
            | (N, N) -> 0
            | (N, S s) when isNull s -> 0
            | (S s, N) when isNull s -> 0
            | (D d1, D d2) -> Decimal.Compare(d1, d2)
            | (B b1, B b2) -> compare b1 b2
